...
class NightDrivingValLoader(data.Dataset):
	def __init__(self, root, img_list_path, lbl_list_path, max_iters=None, crop_size=None, mean=(128, 128, 128), transform=None, return_name = False):
		self.n_classes = 19
		self.root = root
		...
		for img_name, lbl_name in zip(self.img_ids, self.lbl_ids):
			#print(img_name)
			#print(lbl_name)
			img_file = osp.join(self.root,"leftImg8bit/test/night/%s" % (img_name))
			lbl_file = osp.join(self.root,"gtCoarse_daytime_trainvaltest/test/night/%s" % (lbl_name))
