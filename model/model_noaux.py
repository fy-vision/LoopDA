...
'''
class ImgDecoder_SPADE_Seg(nn.Module):
    def __init__(self, bottleneck_channel, dim=256, output_dim=3, n_upsample=3, n_res=3, pad_type='zero', num_classes=19):
        super(ImgDecoder_SPADE_Seg, self).__init__()
        self.bottleneck_channel = bottleneck_channel
        #self.first_conv = Conv2dBlock(self.bottleneck_channel, 256, 3, stride=1, padding=1, norm='in', activation='relu', pad_type='reflect')
        self.model = []
        self.model += [SPADEConv2dBlock(self.bottleneck_channel, dim, 3, pw=1, num_classes=num_classes)]
        self.model += [SPADEResBlocks(n_res, dim, num_classes=num_classes, pad_type=pad_type)]
        # upsampling blocks
        for i in range(n_upsample):
            self.model += [SPADEConv2dBlock(dim, dim // 2, 3, pw=1, num_classes=num_classes, upsample_in=True)]
            dim //= 2
    ...
    def forward(self, x):
        shared_feature, segmap = x[0], x[1]
        _, _, H, W = shared_feature.size()
        shared_feature = F.interpolate(shared_feature, size=(H-1, W-1), mode='bilinear', align_corners=True)
        #shared_feature = self.first_conv(shared_feature)
        out_fea = self.model([shared_feature, segmap])
        return self.conv_rgb(out_fea[0])
'''
class ImgDecoder_SPADE_Seg(nn.Module):
    def __init__(self, bottleneck_channel, dim=256, output_dim=3, n_upsample=3, n_res=3, pad_type='reflect', num_classes=19):
        super(ImgDecoder_SPADE_Seg, self).__init__()
        self.bottleneck_channel = bottleneck_channel
        #self.first_conv = Conv2dBlock(self.bottleneck_channel, 256, 3, stride=1, padding=1, norm='in', activation='relu', pad_type='reflect')
        self.model = []
        self.model += [Conv2dBlock(self.bottleneck_channel, 256, 3, stride=1, padding=1, norm='in', activation='relu', pad_type='reflect')]
        self.model += [ResBlocks(n_res, dim, norm='in', activation='relu', pad_type=pad_type)]
        self.model = nn.Sequential(*self.model)
        # upsampling blocks
        self.model2 = []
        for i in range(n_upsample):
            self.model2 += [SPADEConv2dBlock(dim, dim // 2, 3, pw=1, num_classes=num_classes, upsample_in=True)]
            dim //= 2
        # use reflection padding in the last conv layer
    ...
    def forward(self, x):
        ...
        shared_feature = F.interpolate(shared_feature, size=(H-1, W-1), mode='bilinear', align_corners=True)
        #shared_feature = self.first_conv(shared_feature)
        feat = self.model(shared_feature)
        out_fea = self.model2([feat, segmap])
        return self.conv_rgb(out_fea[0])
